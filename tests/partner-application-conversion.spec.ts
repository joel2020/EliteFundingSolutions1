...
test.describe('partner application conversion hardening', () => {
  test('requires uploaded partner applications to be reviewed before Elite PDF conversion', () => {
    ...
    expect(uploadRoute).toContain("status: 'draft_ready'");
    expect(uploadRoute).toContain('convertedDocument: null');
    expect(uploadRoute).not.toContain('generateLenderApplicationPdf');
    expect(uploadRoute).not.toContain('applicationSyncError');
