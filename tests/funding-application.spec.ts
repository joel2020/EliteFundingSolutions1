...
test.describe('public funding application', () => {
  test('submits the simplified funding-options application', async ({ page }) => {
    ...
    await page.getByTestId('application-business-street-address').fill('10 Main Street');
    await page.getByTestId('application-city').first().fill('New York');
    await page.getByTestId('application-state').first().fill('NY');
    await page.getByTestId('application-zip-code').first().fill('10001');
    await page.getByTestId('application-business-ein-tax-id').fill('123456789');
    await page.getByTestId('application-business-start-date').fill('2021-01-15');
    await page.getByTestId('application-industry-business-type').fill('Retail');
    await page.getByTestId('application-average-monthly-revenue').fill('50000');
    await page.getByTestId('application-amount-requested').fill('75000');
    ...
    await page.getByTestId('application-home-street-address').fill('20 Broadway');
    await page.getByTestId('application-city').last().fill('New York');
    await page.getByTestId('application-state').last().fill('NY');
    await page.getByTestId('application-zip-code').last().fill('10002');
    ...
    await page.getByTestId('add-second-owner').click();
    await page.getByTestId('application-co-owner-full-name').fill('Jordan Reed');
    await page.getByTestId('application-co-owner-home-street-address').fill('40 Partner Ave');
    await page.getByTestId('application-co-owner-city').fill('Brooklyn');
    await page.getByTestId('application-co-owner-state').fill('NY');
    await page.getByTestId('application-co-owner-zip-code').fill('11201');
    ...
    await page.getByTestId('application-lender-funding-company').fill('Old Advance Co');
    await page.getByTestId('application-current-balance').fill('12500');
    await page.getByTestId('application-lender-2-funding-company').fill('Second Capital');
    await page.getByTestId('application-current-balance-2').fill('8000');
    await page.getByTestId('application-lender-3-funding-company').fill('Third Advance');
    await page.getByTestId('application-current-balance-3').fill('4500');
    ...
    await expect(disclosureText).toHaveCSS('color', 'rgb(15, 23, 42)');
    await expect(disclosureText).toHaveCSS('font-size', '13px');
    ...
    await page.getByLabel(/E-Sign and TCPA Consent/).check();
    ...
    expect(submissionCall?.body).toEqual(expect.objectContaining({
      ...
      co_owner_home_address: '40 Partner Ave',
      co_owner_city: 'Brooklyn',
      co_owner_state: 'NY',
      co_owner_zip: '11201',
      co_owner_ssn: '987654321',
      co_owner_dob: '1987-07-12',
      co_owner_cell_phone: '7185550166',
      ...
      existing_advance_balance: '12500',
      existing_advance_2_funder: 'Second Capital',
      existing_advance_2_balance: '8000',
      existing_advance_3_funder: 'Third Advance',
      existing_advance_3_balance: '4500',
    }));
  });
  ...
  test('keeps disclosure text readable on mobile', async ({ page }) => {
    ...
    await expect(mobileDisclosure).toHaveCSS('font-size', '14px');
    await expect(mobileDisclosure).toHaveCSS('color', 'rgb(15, 23, 42)');
  });
});
