    ...
    funding_partners: [
      {
        ...
        preferred_industries: ['retail', 'restaurant'],
        min_credit_score: 600,
        max_existing_positions: 3,
        max_negative_days: 5,
        max_nsf_count: 4,
        preferred_submission_method: 'email',
        criteria_notes: 'Requires voided check for MCA files.',
        created_at: now,
      },
    ],
    iso_brokers: isIso ? [{
  ...
  await page.route('**/api/crm/deals/*/partner-applications', async (route) => {
    ...
    const upload = {
      ...
      extracted_payload: {
        ...
        legal_name: 'Atlas Retail LLC',
        requested_amount: 75000,
        business_phone: '2125550123',
        owner2: { first_name: 'Riley', last_name: 'Partner', ownership_percentage: '25', ssn: '999887777' },
        existing_advances: [{ funder_name: 'Fast Fund', current_balance: '$12,000', daily_payment: '$350' }],
      },
      edited_payload: {
        company_name: 'Atlas Retail LLC',
        legal_name: 'Atlas Retail LLC',
  ...
  await page.route('**/api/crm/partners**', async (route) => {
    ...
    if (route.request().method() === 'PATCH') {
      ...
      const update = {
        ...
        preferred_submission_method: payload.preferred_submission_method || null,
        product_types: String(payload.product_types || '').split(',').map((item) => item.trim()).filter(Boolean),
        preferred_industries: String(payload.preferred_industries || '').split(',').map((item) => item.trim()).filter(Boolean),
        ...
        min_credit_score: payload.min_credit_score ? Number(payload.min_credit_score) : null,
        max_existing_positions: payload.max_existing_positions ? Number(payload.max_existing_positions) : null,
        max_negative_days: payload.max_negative_days ? Number(payload.max_negative_days) : null,
        max_nsf_count: payload.max_nsf_count ? Number(payload.max_nsf_count) : null,
        ...
        criteria_notes: payload.criteria_notes || null,
