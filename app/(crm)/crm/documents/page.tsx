...
export default function DocumentsPage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <CrmTopbar title="Documents" subtitle={`${filteredDocuments.length} documents`} actions={<Button data-testid="upload-document" onClick={() => setShowUploadDialog(true)}><Upload className="w-4 h-4 mr-2" />Upload Document</Button>} />
      ...
      <Dialog open={showUploadDialog} onOpenChange={setShowUploadDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle>Upload Document</DialogTitle><DialogDescription>Upload a PDF or image to the private application-documents bucket. Access uses short-lived signed URLs only.</DialogDescription></DialogHeader>
          <div className="space-y-4 py-4">
            <div><Label htmlFor="file">File *</Label><Input data-testid="document-file" id="file" type="file" onChange={handleFileChange} accept=".pdf,.jpg,.jpeg,.png,.heic,.heif" />{file && <p className="text-xs text-[#71717A] mt-1">Selected: {file.name} ({formatFileSize(file.size)})</p>}</div>
            <div><Label htmlFor="doc_type">Document Type</Label><Select value={docType} onValueChange={setDocType}><SelectTrigger data-testid="document-type"><SelectValue /></SelectTrigger><SelectContent>{docTypes.map((type) => <SelectItem key={type.value} value={type.value}>{type.label}</SelectItem>)}</SelectContent></Select><p className="mt-1 text-xs text-[#71717A]">Public application uploads named bank_statements are normalized to Bank Statement in CRM.</p></div>
            <div><Label htmlFor="description">Description (Optional)</Label><Input data-testid="document-description" id="description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Brief description..." /></div>
          </div>
          <DialogFooter><Button variant="outline" onClick={() => setShowUploadDialog(false)}>Cancel</Button><Button data-testid="save-document" onClick={uploadDocument} disabled={uploading || !file}>{uploading ? 'Uploading...' : 'Upload'}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
