...
import { AlertTriangle, ClipboardList, FileText, RefreshCw, Send, Sparkles } from 'lucide-react';
...
export function DealAiAnalysisPlaceholder({ dealId }: { dealId: string }) {
  ...
  const analysis = useMemo(() => {
    ...
    const hasFunderSubmissions = state.submissions.length > 0;
    ...
    return {
      ...
      suggestedNextSteps: missingDocuments.length
        ...
        : hasFunderSubmissions
          ? 'Review funder responses and update offer or follow-up status.'
          : 'File appears package-ready from current CRM records. Select funders and send the package.',
      riskSummary: redFlags.length
        ? 'CRM records show items that should be reviewed by staff before funder submission.'
  ...
  const generateAiAnalysis = async () => {
    ...
    try {
      ...
      if (result.warning) setAiError(result.warning);
    } catch (err) {
      setAiError(err instanceof Error ? err.message : 'Unable to generate AI analysis.');
    } finally {
      setAiLoading(false);
    }
  };
      ...
      <div className="rounded-[8px] border border-[#CBD5E1] bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start">
            ...
            <Button data-testid="generate-ai-analysis" onClick={generateAiAnalysis} disabled={aiLoading || loading} className="h-9 rounded-[7px] bg-[#0F2B5B] text-xs">
              {aiLoading ? <RefreshCw className="mr-2 h-3.5 w-3.5 animate-spin" /> : <Sparkles className="mr-2 h-3.5 w-3.5" />}
              {aiLoading ? 'Generating' : aiAnalysis ? 'Refresh AI' : 'Generate AI'}
            </Button>
          </div>
        </div>
            ...
            {aiAnalysis ? (
              <div className="mt-4 grid gap-3 lg:grid-cols-4">
                <div className="rounded-[8px] border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                  ...
                  <p className="mt-2 text-sm font-medium text-[#0F172A]">{aiAnalysis.summary}</p>
                  <p className="mt-2 text-xs text-[#64748B]">Confidence: {aiAnalysis.confidence}</p>
                </div>
                <div className="rounded-[8px] border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                  <div className="flex items-center gap-2 text-[11px] font-semibold uppercase text-[#64748B]"><AlertTriangle className="h-3.5 w-3.5" />Risk and readiness</div>
                  <p className="mt-2 text-sm font-medium text-[#0F172A]">{aiAnalysis.fundingReadiness}</p>
                  <p className="mt-2 text-xs text-[#64748B]">{aiAnalysis.riskFlags.length ? aiAnalysis.riskFlags.join('; ') : 'No risk flags returned'}</p>
                </div>
                <div className="rounded-[8px] border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                  <div className="flex items-center gap-2 text-[11px] font-semibold uppercase text-[#64748B]"><ClipboardList className="h-3.5 w-3.5" />Next actions</div>
                  <p className="mt-2 text-sm font-medium text-[#0F172A]">{aiAnalysis.recommendedNextActions.join(' ') || 'No next actions returned.'}</p>
                  <p className="mt-2 text-xs text-[#64748B]">Missing: {aiAnalysis.missingItems.length ? aiAnalysis.missingItems.join(', ') : 'None listed'}</p>
                </div>
                <div className="rounded-[8px] border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                  <div className="flex items-center gap-2 text-[11px] font-semibold uppercase text-[#64748B]"><Send className="h-3.5 w-3.5" />Funder email draft</div>
                  <p className="mt-2 text-sm font-semibold text-[#0F172A]">{aiAnalysis.funderEmailDraft.subject}</p>
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-3">
                ...
                <div className="rounded-[8px] border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                  ...
                  <p className="mt-2 text-xs text-[#64748B]">Use Generate AI when the file is ready for a staff-reviewed package summary.</p>
                </div>
              </div>
            )}
            {aiAnalysis?.questionsForMerchant?.length ? (
              <div className="mt-3 rounded-[8px] border border-[#E2E8F0] bg-white p-3 text-xs text-[#475569]">
                <b className="text-[#0F172A]">Merchant follow-up questions:</b> {aiAnalysis.questionsForMerchant.join(' ')}
              </div>
            ) : null}
          </div>
        )}
