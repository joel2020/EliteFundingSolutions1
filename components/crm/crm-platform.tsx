...
export function CrmDealDetailExperience({ dealId }: { dealId: string }) {
  ...
  const openLenderSubmission = () => {
    ...
    const requiredTypes = lenderRequiredDocTypes(partner);
    const presetIds = requiredTypes.map((type) => bestDocumentForType(dealDocs, type)?.id).filter(Boolean) as string[];
    setSubmissionPartnerId(partnerId);
    setSubmissionDocumentIds(Array.from(new Set(presetIds)));
          ...
          <TabsContent value="applications">
            <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-xs text-[#64748B]">Upload a partner application once. The CRM keeps the original, creates the Elite Funding application, and includes it when sending to lenders.</p>
              </div>
            ...
            <div className="mb-4 grid gap-3 md:grid-cols-3">
              ...
              <CrmCard className="p-3"><p className="text-[11px] font-semibold uppercase text-[#64748B]">Elite application</p><div className="mt-2"><StatusBadge value={convertedApplicationDocs.length ? 'ready_to_send' : 'not_started'} /></div><p className="mt-2 text-xs text-[#64748B]">Ready PDFs: {convertedApplicationDocs.length}</p><p className="text-xs text-[#64748B]">{convertedApplicationDocs.length ? 'Latest converted app is auto-included for lender submissions.' : 'Upload a partner app or generate an Elite PDF.'}</p></CrmCard>
            </div>
      ...
      <Dialog open={submissionDialogOpen} onOpenChange={setSubmissionDialogOpen}>
        <DialogContent className="max-w-2xl rounded-[8px]">
          ...
          <div className="grid gap-4">
            <div><Label className="text-xs text-[#64748B]">Funding partner</Label><Select value={submissionPartnerId} onValueChange={selectSubmissionPartner}><SelectTrigger data-testid="deal-submission-partner" className="mt-1 rounded-[7px]"><SelectValue placeholder="Select a lender" /></SelectTrigger><SelectContent>{partners.map((partner: RecordMap) => <SelectItem key={partner.id} value={partner.id}>{partner.name}</SelectItem>)}</SelectContent></Select></div>
            ...
            {selectedSubmissionPartner && <div data-testid="lender-preset-summary" className={`rounded-[8px] border p-3 text-sm ${selectedPartnerMissingDocTypes.length ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-emerald-200 bg-emerald-50 text-emerald-950'}`}><b>Lender package preset:</b> {submissionDocumentIds.length} attachment(s) preselected. {selectedPartnerMissingDocTypes.length ? `Missing: ${selectedPartnerMissingDocTypes.map(detailDocTypeLabel).join(', ')}.` : 'Required package looks complete.'}</div>}
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-[#64748B]">The latest Elite converted application is included automatically. If none exists, the CRM generates one before sending. Select any additional statements or documents to include.</p>
            </div>
          </div>
