...
export function CrmDealDetailExperience({ dealId }: { dealId: string }) {
  ...
  const uploadPartnerApplication = async () => {
    ...
    try {
      ...
      toast.success('Partner application converted and ready for funders');
          ...
          <TabsContent value="applications">
            <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-xs text-[#64748B]">Upload a partner application once. The CRM keeps the original, creates the Elite Funding application, and includes it when sending to funders.</p>
              </div>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="mb-4 grid gap-3 md:grid-cols-3">
              ...
              <CrmCard className="p-3"><p className="text-[11px] font-semibold uppercase text-[#64748B]">Elite application</p><div className="mt-2"><StatusBadge value={convertedApplicationDocs.length ? 'ready_to_send' : 'not_started'} /></div><p className="mt-2 text-xs text-[#64748B]">Ready PDFs: {convertedApplicationDocs.length}</p><p className="text-xs text-[#64748B]">{convertedApplicationDocs.length ? 'Latest converted app is auto-included for funder submissions.' : 'Upload a partner app or generate an Elite PDF.'}</p></CrmCard>
            </div>
