...
export function useCrmDataset() {
  ...
  const load = useCallback(async () => {
    ...
    const settled = await Promise.allSettled([
      ...
      browserSupabase.from('applications').select('*').eq('organization_id', org).is('deleted_at', null).order('created_at', { ascending: false }).limit(500),
      browserSupabase.from('owners').select('*').eq('organization_id', org).is('deleted_at', null).order('created_at', { ascending: false }).limit(500),
